%--------------------------------------------------------------------------
function [codebook] = generateCodebook(codebookMode)
...
nrOfKmeansClusters = 56;                % 56 number of Clusters for K-Means algorithm
...
if (strcmp(codebookMode,'lbg'))
    ...
    e = .000000001;
    for p=1:nrOfUsers
        A=getMFCC(allusers.(cellContent{p}).sample,nrOfMfccCoeffs,'wav');
        ...
        for i = 1:log2(k)
            ...
            while (true) % do while-loop
                ...
                if (((dpr - t)/t) < e)
                    allusers.(cellContent{p}).characteristics=r;
...
elseif(strcmp(codebookMode,'kmeans'))
    ...
    rng(1)
    for p=1:nrOfUsers
        A=getMFCC(allusers.(cellContent{p}).sample,nrOfMfccCoeffs,'vect');
        %dirty removal of NaN column -> to be improved
        if sum(isnan(A(1,:)))>0
            A=A(:,1:length(A(1,:))-1);
        end
